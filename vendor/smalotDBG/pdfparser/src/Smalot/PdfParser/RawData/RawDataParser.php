...
class RawDataParser
{
    ...
    protected function decodeStream(string $pdfData, array $xref, array $sdic, string $stream): array
    {
        ...
        #Si dentro de la cadena que se inserto no se encontro un / entonces decodifica la cadena
        // decode the stream
        $remaining_filters = [];
        foreach ($filters as $filter) {
            #Lo siguinete comprueba que los filtros soportados coincidan con el filtro  ['ASCIIHexDecode', 'ASCII85Decode', 'LZWDecode', 'FlateDecode', 'RunLengthDecode'];
            #Lo que indiica que el filtro si se encuentra.
            if (\in_array($filter, $this->filterHelper->getAvailableFilters())) {
                try {
                    #Una vez encontrado el filtro entonces decodifica la cadena. con un limite de memoria de 0
                    $stream = $this->filterHelper->decodeFilter($filter, $stream, $this->config->getDecodeMemoryLimit());
                    #!!ACA NO SE ENCUENTRA EL ERROR DE SECUENCIA LA CADENA DE CODIFICADA ES LA MISMA
                    echo $stream;
                } catch (Exception $e) {
                    $emsg = $e->getMessage();
